...
export class ComputedSignal {
  protected person1 = signal<string>('Oak Soe Aung');
  protected person2 = signal<string>('Htet Hnin Oo');

  protected person1Amount = signal<number>(1000);
  protected person2Amount = signal<number>(1000);

  protected total = computed(() => this.person1Amount() + untracked(this.person2Amount));
  /*protected total = computed(() => this.person1Amount() + untracked(this.person2Amount));*/ // person2Amount change ရင် re-calculate မလုပ်ဘူး person1Amount change မှ re-calculate ပြန်လုပ်တယ်
